...
export const ButtonComponent = ({link,address,text,img,icon,textColor,colorBoder,colorBg,bold,padding,hover,height,fullScreen,width,colorHover,notHover,cursor}) => {
  return (
      <button className={`${fullScreen?`w-full`:""} ${cursor?cursor:""} ${width?width:""} flex items-center justify-center border ${bold?`font-bold`:""} ${colorBg?`bg-${colorBg}`:""} ${colorBoder?`border-${colorBoder}`:"border-first"} ${textColor?`text-${textColor}`:"text-black"} ${padding?`px-${padding}`:"px-4"} ${height?height:"h-10"} ${notHover?``:colorHover?`hover:bg-${colorHover}`:hover?"hover:bg-[#1739531f]":"hover:bg-[#3e4143]"}`}>
          {icon}
