...
import { getListCourse } from '../../services/courses';
import { Link } from 'react-router-dom';
import { BoxContent2 } from '../../components/BoxContent/BoxContent2';
  ...
  return (
    <div>
      ...
      <div className="mt-8 px-5">
          <BoxContent2 block1={<><BoxContent img={"assets/logos/logo3.svg"} h1={t("Nâng cao kỹ năng cho đội ngũ nhân viên của bạn với Udemy Business")}
            li={[
              t("Truy cập không giới hạn hơn 27000 khóa học hàng đầu của Udemy, ở mọi nơi và mọi lúc"),
              t("Tuyển tập khóa học quốc tế bằng 14 ngôn ngữ"),
              t("Các chứng chỉ hàng đầu về công nghệ và kinh doanh"),
            ]}  
          />
          <div className='flex gap-2 mt-4 text-base tablet:justify-center'>
            <ButtonComponent fullScreen={true} height={"h-12"} bold={true} text={t("Hãy tham gia Udemy Business")} colorBg={"first"} textColor={"white"}/>
            <ButtonComponent fullScreen={true} height={"h-12"} bold={true} text={t("Tìm hiểu thêm")} hover={true}/>
          </div>
          </>}
          block2={ <img className='h-full' src="assets/imgs/img0.jpg" alt="" />}
          />
      </div>
      <div className='mt-16 mb-24 px-5'>
        <BoxContent2 type={2} block1={<BoxContent  h1={t("Trở thành giảng viên")} p={t("Giảng viên trên khắp thế giới giảng dạy hàng triệu học viên trên Udemy. Chúng tôi cung cấp công cụ và kỹ năng để dạy những gì bạn yêu thích.")} textBn={t("Bắt đầu dạy học ngày hôm nay")} heightBn={"h-12"}/>}
          block2={ <img className='h-full' src="assets/imgs/img1.jpg" alt="" />}
          />
      </div>
    </div>
  )
